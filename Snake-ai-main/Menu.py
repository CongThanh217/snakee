...
class MainMenu(Menu):
    ...
    def display_menu(self):
        ...
        self.game.display = pygame.display.set_mode(WINDOW_SIZE)
        while self.run_display:
            self.game.event_handler()
            self.check_input()

            # Replace "background.jpg" with the actual path to your image file
            self.game.display.blit(background, (0, 0))  # Blit the background at coordinates (0, 0)
            # self.game.draw_text(
...
class GAMenu(Menu):
    def __init__(self, game, controller):
        ...
        self.train_model = button(
            game.SIZE / 2 - 4 * (CELL_SIZE + NO_OF_CELLS), game.SIZE / 2 + 3.5 * (CELL_SIZE + NO_OF_CELLS),
            'Train Model', game)
